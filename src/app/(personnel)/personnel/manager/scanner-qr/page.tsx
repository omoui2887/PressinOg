 ...
 *
 * La redirection va vers /personnel/manager/commandes/{id} (page détail
 * variante manager, qui réutilise <CommandeDetail basePath=... />).
 *
 * 🚫 PLAN GATING (PRD §16) :
  ...
  async function handleScanSuccess(decoded: string) {
    ...
    if (searchQuery) {
      try {
        ...
        console.error("[manager/scanner-qr] Erreur lookup scan:", err);
      }
    }
