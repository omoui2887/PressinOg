 ...
 *
 * Fetch côté serveur via `getSupabaseServer()` (même logique que
 * GET /api/admin/commandes/[id]). RLS isole par pressing : si la commande
 * n'existe pas ou n'appartient pas au pressing, la SELECT renvoie null →
 * page 404 avec message.
 *
...
export default async function CommandeDetailPage({ params }: PageProps) {
  ...
  if (cmdErr) {
    console.error("[admin/commandes/[id]] Erreur SELECT:", cmdErr);
  }
  ...
  return <CommandeDetail commande={detail} />;
}
